...
def safe_notch(sig, sfreq, mains=50.0, bw=4.0):
    """安全陷波：优先用 remove_environmental_noise；否则手工带阻。"""
    nyq = sfreq / 2.0
...
def bandpower_welch(sig, sfreq, fmin, fmax):
    """稳健 Welch：至少两段（50%重叠），否则回退 periodogram。"""
    n = len(sig)
    if n < 32:
        return 0.0
    max_base = max(32, n // 2)
...
def _robust_center_scale(x):
    ...
    sigma = 1.4826 * mad  # 约等于标准差
    return med, max(sigma, 1e-12)

def compute_fp_features(sig, sfreq):
    """对单通道 Fp：去趋势 + 低通 8Hz，返回 峰-峰值 与 1–4Hz 波段功率"""
...
class BlinkGate:
    ...
    def is_blink(self, fp_mat, sfreq, now_ts):
        """
        fp_mat: (n_fp, n_samples)，包含 Fp1/Fp2（至少其一）
        """
        ...
        pp = float(np.max(pp_vals))   # 取更像眨眼的一侧
...
def eog_regress_out(use_mat, eeg_mat, fp_rel_idx):
    """Fp1/Fp2 作为回归自变量，从 use_mat（Fz/Cz）回归去除眼动成分。"""
    ...
    Xc = np.vstack([X, np.ones((1, X.shape[1]))])  # 加常数项 (3, n)
    Xt = Xc.T  # (n, 3)
...
def compute_tbr(block_2d, sfreq):
    """block_2d: (n_used_channels, n_samples) —— 每通道算 theta/beta，再取平均"""
    ratios = []
...
def render_bar(ratio, threshold, blink=False, noisy=False):
    """终端进度条；ratio < threshold 为“好”。眨眼/噪声时不显示 ✔。"""
...
def main():
    ...
    try:
        ...
        while time.time() - baseline_start < BASELINE_SEC:
            ...
            eeg_mat = buf[eeg_channels, :]   # (n_eeg, n_samples)
        ...
        while time.time() - train_start < TRAIN_SEC:
            ...
            # 统计：眨眼/噪声窗口不计入 succ/tot，也不播 success
